...
public final class MockitoAbstractControllerTest {
	...
	Mockito.doCallRealMethod().when(this.controller)
	...
	Mockito.when(((AbstractController<String>) this.controller) //
		.getModel(Matchers.anyString())).thenReturn(MODEL);

	Mockito.when(this.service.read(Matchers.anyInt())).thenReturn(MODEL);
    }
